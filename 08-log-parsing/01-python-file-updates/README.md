...
## Overview
In this activity, I built a Python algorithm to maintain an allow list of IP addresses used to control access to restricted patient-record systems. The goal was to remove IP addresses listed in a separate `remove_list` from the allow list stored in `allow_list.txt`. The script reads the file, parses its content into a list, removes unauthorized IPs, and writes the updated allow list back to the file. :contentReference[oaicite:0]{index=0}
...
- Overwrote `allow_list.txt` with the revised allow list using `.write()` :contentReference[oaicite:1]{index=1}
...
- List updates: `.remove()` (valid here because the allow list contains no duplicates) :contentReference[oaicite:2]{index=2}
